...
class MetaAnalysisInput(BaseWorkflowInput):
    target_case_ids: list[str] = Field(
        description="List of Case IDs to perform cross-case meta-analysis on"
    )
    timeframe_days: int = Field(
        default=30, description="Analysis timeframe window in days"
    )
...
def extract_meta_analysis_payload_node(
    inp: MetaAnalysisInput,
) -> ExtractedMetaAnalysisPayload:
...
def synthesize_cross_case_patterns_node(
    payload: ExtractedMetaAnalysisPayload,
) -> MetaAnalysisSynthesisResult:
    has_systemic = len(payload.target_case_ids) >= 3
    return MetaAnalysisSynthesisResult(
        payload=payload,
        cross_case_common_entities=["admin-svc-account", "198.51.100.44"]
        if has_systemic
        else [],
...
def handle_systemic_risk_branch(
    synthesis: MetaAnalysisSynthesisResult,
) -> MetaAnalysisOutcome:
...
def handle_isolated_incidents_branch(
    synthesis: MetaAnalysisSynthesisResult,
) -> MetaAnalysisOutcome:
