...
- **Mandatory Initial Telemetry Retrieval:** If you are triaging an alert or case, you MUST immediately execute `get_case_alert` (for alerts) or `get_case` (for cases) as your very first tool call. You MUST extract all embedded telemetry fields from the raw payload, specifically checking for a `processTree` (which details the process hierarchy like root parent processes) or list of involved users/hosts. You MUST use these extracted entities and trees to guide your subsequent lookups and timeline reconstruction. Never skip this initial retrieval.
