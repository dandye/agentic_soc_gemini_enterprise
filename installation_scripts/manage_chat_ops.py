...
class ChatOpsManager:
    ...
    def __init__(self, env_file: Path):
        ...
        self.region = self.env_vars.get("GCP_LOCATION", "us-east4")  # Standard for this project
    ...
    def list_cards(self, cards_dir: Path):
        ...
        if not cards_dir.exists():
            typer.secho(f"Error: Cards directory {cards_dir} not found.", fg=typer.colors.RED)
            return []

        py_stems = {c.stem for c in cards_dir.glob("*.py") if c.name != "__init__.py" and c.name != "card_client.py"}
        sh_stems = {c.stem for c in cards_dir.glob("*.sh") if c.name != "refactor_sh_files.sh"}
        
        all_stems = sorted(list(py_stems | sh_stems))
    ...
    def _test_py_card(self, stem: str, cards_dir: Path):
        ...
        typer.echo(f"Sending card (Python): {typer.style(py_path.name, fg=typer.colors.CYAN)}")
        ...
        try:
            ...
            if not hasattr(module, "get_card"):
                typer.secho(f"Error: {py_path.name} does not have a get_card() function.", fg=typer.colors.RED)
                return False
    ...
    def _test_sh_card(self, sh_path: Path, cards_dir: Path):
        ...
        typer.echo(f"Sending card (Shell fallback): {typer.style(sh_path.name, fg=typer.colors.YELLOW)}")
        try:
            env = os.environ.copy()
    ...
    def deploy_backend(self, function_dir: Path, language: str = "python", service: str = "chatops-handler"):
        """Deploy the backend Cloud Function."""
        if not function_dir.exists():
            typer.secho(f"Error: Function directory {function_dir} not found.", fg=typer.colors.RED)
            return False
        ...
        try:
            ...
            typer.secho("Warning: Could not fetch project number, some logic might fail.", fg=typer.colors.YELLOW)
            project_number = "0"

        success = True
        for entry in entry_points:
            # If service name is provided, use it for the first entry point, then append suffix for others
            func_name = entry if len(entry_points) == 1 else f"{service}-{entry.replace('_', '-')}"
...
def list_cards(
    ...
    cards_dir: Annotated[Path, typer.Option(help="Path to cards directory")] = Path("soc_agent/tools/chatops")
):
...
def deploy(
    env_file: Annotated[Path, typer.Option(help="Path to .env file")] = Path(".env"),
    func_dir: Annotated[Path, typer.Option(help="Path to function source")] = Path("functions_python"),
