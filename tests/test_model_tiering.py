...
def test_agent_model_defaults_in_source_code():
    ...
    mgr_agent = (repo_root / "agent_soc_manager" / "agent.py").read_text(
        encoding="utf-8"
    )
    assert (
    ...
    cti_agent = (repo_root / "agent_a2a_cti_researcher" / "agent.py").read_text(
        encoding="utf-8"
    )
    assert (
        'CTI_RESEARCHER_MODEL = os.environ.get("CTI_RESEARCHER_MODEL", "gemini-3.7-flash")'
    ...
    det_agent = (repo_root / "agent_a2a_detection_engineer" / "agent.py").read_text(
        encoding="utf-8"
    )
    assert (
        'DETECTION_ENGINEER_MODEL = os.environ.get(\n        "DETECTION_ENGINEER_MODEL", "gemini-3.7-flash"\n    )'
        in det_agent
        or 'DETECTION_ENGINEER_MODEL = os.environ.get("DETECTION_ENGINEER_MODEL", "gemini-3.7-flash")'
        in det_agent
    )

    # 4. agent_a2a_threat_hunter/agent.py
    hunter_agent = (repo_root / "agent_a2a_threat_hunter" / "agent.py").read_text(
        encoding="utf-8"
    )
    assert (
    ...
    tier2_agent = (repo_root / "agent_a2a_tier2" / "agent.py").read_text(
        encoding="utf-8"
    )
    assert (
